...
from .migrator import ContextMigrator, SUPPORTED_SOURCES
...
def _quickstart_lines(project_root: Path | None = None) -> list[str]:
    ...
    return [
        ...
        "  - `migrate` extracts IDE chat context into agent-mem memory and a backup.",
...
5. If the user pastes a migration handoff prompt from `agent-mem migrate --full`, execute it immediately before regular chat flow.
...
When a migration handoff prompt is present:

1. Execute the handoff before normal Q&A.
2. Reconcile migrated summary with current memory.
3. If summary conflicts with repository files, call out the conflict and prefer repository state.
4. Save one fresh structured summary back to memory.
5. Return a compact fresh-chat starter block.
...
- the user pastes a migration handoff prompt generated by `agent-mem migrate --full`
...
- If the user pastes a migration handoff prompt from `agent-mem migrate --full`, execute it immediately and do not treat it like an ordinary question.
...
7. If context started in another IDE, run `agent-mem migrate --dry-run` first and then `agent-mem migrate --full ...`.
8. If a migration handoff prompt is pasted, execute it first, save a structured summary, then continue normal implementation.
...
- If the user pastes a migration handoff prompt generated by `agent-mem migrate --full`, execute it immediately.
...
- If the user migrated history from another IDE, treat the migrate handoff as high-priority context.
- If a migration handoff prompt is pasted, execute it first, save a structured summary, then continue normal implementation.
...
- If the user pastes a migration handoff prompt from `agent-mem migrate --full`, execute it immediately.
...
- If history was migrated via `agent-mem migrate`, use migrated summary + backup as continuity context.
...
def _ide_setup_instructions(target: str) -> str:
    instructions = {
        "cursor": "Cursor setup: reload the workspace. agent-mem created `.cursor/rules/agent-mem.mdc` and `.cursor/mcp.json`. Start a fresh chat. If `agent-mem watch` or `agent-mem migrate --full ...` gives you a handoff prompt, paste it into the current Cursor chat first, let Cursor summarize/save memory, then start a fresh chat with the starter block it returns.",
        "claude": "Claude / VS Code setup: reload the workspace. agent-mem created `.claude/instructions.md` and `.vscode/mcp.json`. If your Claude extension does not auto-read `.claude/instructions.md`, paste `AGENT-MEM-RULES.md` into custom instructions once. If `agent-mem watch` or `agent-mem migrate --full ...` gives you a handoff prompt, paste it into the current Claude chat first, let it summarize/save memory, then start a fresh chat.",
        "antigravity": "Antigravity setup: reload the workspace. agent-mem created `.antigravity/rules.md`. If Antigravity needs MCP separately, use `agent-mem print-mcp-json`. If `agent-mem watch` or `agent-mem migrate --full ...` gives you a handoff prompt, paste it into the active chat first and let the agent summarize/save memory before starting fresh.",
        "opencode": "OpenCode setup: reload the workspace. agent-mem created `.opencode/instructions.md`. If OpenCode needs MCP separately, use `agent-mem print-mcp-json` and add it in that product's MCP settings. If `agent-mem watch` or `agent-mem migrate --full ...` gives you a handoff prompt, paste it into the active chat first and let the agent handle the summary/handoff.",
...
def _normalize_migration_sources(raw_values: List[str]) -> List[str]:
    normalized: List[str] = []
    seen: set[str] = set()
    aliases = {
    ...
    for raw in raw_values:
        for token in raw.split(","):
            value = aliases.get(token.strip().lower(), token.strip().lower())
            if not value:
                continue
            if value == "all":
                for source in ("cursor", "claude", "opencode"):
...
def _parse_migrate_targets(targets: List[str], default_root: Path) -> tuple[List[str], Path]:
    if not targets:
        return [], default_root

    root = default_root
    source_tokens = list(targets)
    last = Path(targets[-1]).expanduser()
    if last.exists():
        resolved = last.resolve()
        root = resolved if resolved.is_dir() else resolved.parent
        source_tokens = targets[:-1]

    return _normalize_migration_sources(source_tokens), root


def _prompt_migration_sources() -> List[str]:
    default_sources = "cursor,claude,opencode"
    prompt = f"Migration sources [{'/'.join(SUPPORTED_SOURCES)}/all]"
    while True:
        raw = _prompt(prompt, default=default_sources)
        normalized = _normalize_migration_sources([raw])
        if normalized:
            return normalized
        _echo(f"❌ Choose at least one valid source: {', '.join(SUPPORTED_SOURCES)} or all", err=True)
...
def init():
    ...
    _echo("  4. agent-mem migrate --dry-run cursor .")
    _echo("  5. agent-mem graph build")
    if written_paths:
...
def migrate(
    targets: List[str] = typer.Argument(
        None,
        help="Sources followed optionally by a project path. Example: agent-mem migrate cursor claude .",
    ),
    full: bool = typer.Option(
        False,
        "--full",
        help="Extract + save summary to memory + generate handoff + write markdown backup.",
    ),
    extract_only: bool = typer.Option(
        False,
        "--extract-only",
        help="Extract chats and write markdown backup only (no memory write or handoff save).",
    ),
    dry_run: bool = typer.Option(
        False,
        "--dry-run",
        help="Run safely without writing memory or backup files.",
    ),
    project_name: str = typer.Option(
        "",
        "--project-name",
        help="Override inferred project name for memory summary writes.",
    ),
    max_sessions: int = typer.Option(
        ...
        min=1,
        max=100,
        help="Maximum sessions to keep after extraction per source.",
    ),
    max_messages: int = typer.Option(
        ...
        min=20,
        max=5000,
        help="Maximum messages retained per extracted session.",
    ),
):
    """Extract IDE chat history into agent-mem summaries, handoff prompts, and backups."""
    if full and extract_only:
        _echo("❌ Use either --full or --extract-only, not both.", err=True)
        raise typer.Exit(1)

    default_root = _project_root()
    interactive_mode = not targets

    if interactive_mode:
        _echo("Interactive migration mode")
        sources = _prompt_migration_sources()
        path_input = _prompt("Project path", default=str(default_root))
        resolved_root = Path(path_input).expanduser().resolve()
        if not resolved_root.exists():
            _echo(f"❌ Project path not found: {resolved_root}", err=True)
            raise typer.Exit(1)
        if resolved_root.is_file():
            resolved_root = resolved_root.parent

        if not full and not extract_only:
            full = _confirm("Run full mode (save memory + generate handoff + backup)?", default=True)
            extract_only = not full
    else:
        sources, resolved_root = _parse_migrate_targets(targets, default_root)
        if not sources:
            _echo(
                f"❌ No valid migration source provided. Use one of: {', '.join(SUPPORTED_SOURCES)} (or all).",
                err=True,
            )
            raise typer.Exit(1)
        if not full and not extract_only:
    ...
    effective_project_name = project_name.strip() or _project_name_from_root(resolved_root)
    ...
    try:
        result = migrator.run(sources=sources, full=full, extract_only=extract_only)
    ...
    _echo(f"Project root   : {resolved_root}")
    _echo(f"Project name   : {effective_project_name}")
    _echo(f"Mode           : {'full' if full else 'extract-only'}")
    _echo(f"Dry run        : {'yes' if dry_run else 'no'}")
    _echo(f"Sources        : {', '.join(result.sources)}")
    _echo(f"Sessions found : {len(result.sessions)}")
    if sessions_by_source:
        _echo("Per-source     : " + ", ".join(f"{name}={count}" for name, count in sorted(sessions_by_source.items())))
    ...
    if dry_run:
        ...
        if full:
            _echo("\nDry run handoff prompt preview:")
            _echo(migrator.generate_handoff_prompt(result.summary, result.sessions))
        _echo("\nNo files were written because --dry-run is enabled.")
        return

    if result.saved_summary_path:
        _echo(f"✅ Memory summary saved: {result.saved_summary_path}")
    if result.backup_path:
        _echo(f"✅ Markdown backup saved: {result.backup_path}")
    if result.handoff_prompt:
        _echo("\nHandoff prompt:")
        _echo(result.handoff_prompt)
