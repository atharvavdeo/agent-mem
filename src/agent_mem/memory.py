...
HANDOFF_OUTBOX_DIR = "outbox"
HANDOFF_OUTBOX_FILE = "latest-handoff.md"
...
def get_handoff_outbox_file(project_root: Path | None = None) -> Path:
    memory_dir = get_memory_dir(project_root)
    outbox_dir = memory_dir / HANDOFF_OUTBOX_DIR
    outbox_dir.mkdir(parents=True, exist_ok=True)
    return outbox_dir / HANDOFF_OUTBOX_FILE
...
def _extract_section_text(summary: str, heading: str) -> str:
    pattern = rf"(?ims)^#+\s*{re.escape(heading)}\s*$\n(.*?)(?=^\s*#|\Z)"
    match = re.search(pattern, summary)
    if not match:
        return ""
...
def _obsidian_note(project_name: str, summary: str, moment: datetime) -> str:
    ...
    task_links = [_wiki_label(f"Task - {item}") for item in _extract_section_items(summary, "Open tasks or blockers")]
    file_links = _extract_file_links(summary)
    goal_text = _extract_section_text(summary, "Goal") or "No explicit goal captured."
    outcome_text = _extract_section_text(summary, "Outcome") or "No explicit outcome captured."
    decisions_block = "\n".join(f"- [[{label}]]" for label in decision_links) or "- None recorded"
    files_block = "\n".join(f"- [[{label}]]" for label in file_links) or "- None recorded"
    tasks_block = "\n".join(f"- [[{label}]]" for label in task_links) or "- None recorded"
    next_steps = _extract_section_items(summary, "Next prioritized steps")
    next_steps_block = "\n".join(f"- {item}" for item in next_steps) or "- None recorded"
...
def _update_obsidian_index(project_name: str, session_note_name: str, project_root: Path | None = None):
    ...
    for link in existing_links:
        if link != new_session_link and link != f"[[{project_link}]]" and link != f"[[{OBSIDIAN_ACTIVE_NAME.removesuffix('.md')}]]":
